...
func aboutSlices() {
	...
	pregnancy_slots = append(pregnancy_slots, "another baby!?")

	assert(len(pregnancy_slots) == 5) // append() can take N arguments to append to the slice
	assert(cap(pregnancy_slots) == 6) // the capacity optimizations have a guessable algorithm

	pregnancy_slots = append(pregnancy_slots, "yet another, oh dear!")

	assert(len(pregnancy_slots) == 6) // append() can take N arguments to append to the slice
	assert(cap(pregnancy_slots) == 6) // the capacity optimizations have a guessable algorithm

	//NJ: when using append, it will allocate double the amount required
	//		since the last realloc. This is a wierd alog.
	pregnancy_slots = append(pregnancy_slots, "they must be Catholic")
